...
设计完成后，AI 调用 `download` 接口，由浏览器将蓝图直接落成磁盘文件，再移动至引擎蓝图库，全程数据不经过模型上下文，避免大体积 JSON 在传输中被截断或抄写走样。导出的蓝图包含标准 JSON 与 `.litematic` 存档两种形式，后者可被 Litematica 模组直接打开。
...
2. **建造阶段**（mc-build）：默认以秒落模式将蓝图一次性写入世界；用户希望观看过程时，切换为表演模式，由工人机器人按预生成的指令清单走位回放。建造完成后执行 verify-region 全量回读验收，蓝图与世界的逐格比对四项计数全部归零方可通过。
3. **修改阶段**（mc-modify）：在既有蓝图上执行差量修改，先计算新旧版本的最小变更集，再仅将差异格写入世界，最后同步真相源并再次验收。纯材质替换提供一键通道。

## 核心原则

- **蓝图即真相源。** 引擎蓝图库中的 JSON 是唯一权威数据，世界中的任何修改都必须回写同步，保证蓝图与实际建筑始终一致。
- **数据不过模型。** 设计产物由浏览器直接落盘，不注入模型上下文；大体量数据在上下文中的往返既浪费资源，也可能引入抄写误差。
- **差量优先。** 修改一律通过最小变更集完成，除用户明确要求外不进行整体重建。
- **全量验收。** 每次建造与修改后执行逐格回读比对，以数据而非目测作为通过标准；观感终审由用户游戏内截图完成。
